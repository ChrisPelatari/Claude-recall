...
┌──────────────────────────────────────────────────────────────────────┐
│                     Entry / App lifecycle                            │
│  `ClaudeRecallApp` (@main, SwiftUI Scene)                          │
│  `AppDelegate` (macOS only — buffers cold-start file URLs)           │
│  `App/ClaudeRecallApp.swift`                                       │
└───────────────┬─────────────────────────────┬────────────────────────┘
                │                             │
                ▼                             ▼
┌──────────────────────────────┐  ┌────────────────────────────────────┐
...
|-----------|----------------|------|
| `ClaudeRecallApp` | `@main` Scene; instantiates `AppState`, wires `.onOpenURL`, registers macOS menu commands | `ClaudeRecall/Sources/App/ClaudeRecallApp.swift` |
| `AppDelegate` (macOS) | Restores bookmarks on launch, stops scopes on terminate, buffers cold-start `open:` URLs until SwiftUI is ready | `ClaudeRecall/Sources/App/ClaudeRecallApp.swift` |
| `AppState` | Single `@Observable @MainActor` hub for all UI state, source selection, search, file watching, and URL-scheme dispatch | `ClaudeRecall/Sources/Models/AppState.swift` |
| `AISource` | Static catalog of 8 supported AI agents + custom user-added sources; detects which exist on disk and contain supported files | `ClaudeRecall/Sources/Models/AISource.swift` |
| `FileNode` | `@Observable` reference-type tree node for the sidebar (id is its absolute path) | `ClaudeRecall/Sources/Models/FileNode.swift` |
| `SettingsStore` | iCloud-KV + UserDefaults dual-write for synced preferences | `ClaudeRecall/Sources/Models/SettingsStore.swift` |
| `AppTheme` / `ThemePalette` | Theme enum + per-theme color palette resolved via `.environment(\.themePalette, …)` | `ClaudeRecall/Sources/Models/AppTheme.swift` |
| `BookmarkStore` | Security-scoped bookmark persistence (UserDefaults), runtime sandbox detection | `ClaudeRecall/Sources/Utilities/BookmarkStore.swift` |
| `FileTreeBuilder` | Pure function: build a `FileNode` tree from a root URL, filtering to `.md`/`.json` and pruning empty dirs | `ClaudeRecall/Sources/Utilities/FileTreeBuilder.swift` |
| `FileWatcher` (macOS) | FSEvents stream wrapper that posts `.fileWatcherDidDetectChange` | `ClaudeRecall/Sources/Utilities/FileWatcher.swift` |
| `SearchService` | Substring search across all `.md`/`.json` under a root, or within a single file | `ClaudeRecall/Sources/Utilities/SearchService.swift` |
| `UpdateChecker` | Daily GitHub Releases poll, semver compare, dismissed-version persistence | `ClaudeRecall/Sources/Utilities/UpdateChecker.swift` |
| `PDFExporter` (macOS) | Renders a `Markdown` view through `NSHostingView` + `NSPrintOperation` to produce a PDF | `ClaudeRecall/Sources/Utilities/PDFExporter.swift` |
| `WindowChrome` (macOS) | Tints the title bar to match the current theme | `ClaudeRecall/Sources/Utilities/WindowChrome.swift` |
| `MemoryReaderTheme` | Builds a `MarkdownUI.Theme` from a `ThemePalette` | `ClaudeRecall/Sources/Utilities/MemoryReaderTheme.swift` |
| `SplashCodeSyntaxHighlighter` | Splash for Swift; hand-rolled keyword highlighter for Python/JS/JSON/Bash/YAML/Markdown | `ClaudeRecall/Sources/Utilities/SplashCodeSyntaxHighlighter.swift` |
| `LocalImageProvider` (macOS) | Resolves relative/absolute markdown image refs against the document's parent directory | `ClaudeRecall/Sources/Utilities/LocalImageProvider.swift` |
| `ReadableMarkdownRenderer` (macOS) | Converts markdown → `NSAttributedString` for find-in-page highlighting in an `NSTextView` | `ClaudeRecall/Sources/Utilities/ReadableMarkdownRenderer.swift` |
| `ContentView` | Platform router → `MacContentView` (`NavigationSplitView`) or `iOSContentView` (`NavigationStack`) | `ClaudeRecall/Sources/Views/ContentView.swift` |
| `SidebarView` (macOS) | AI source picker + search + file tree | `ClaudeRecall/Sources/Views/SidebarView.swift` |
| `DetailView` / `MarkdownDetailView` (macOS) | Renders selected markdown/JSON, hosts find bar + TOC + edit mode + PDF export | `ClaudeRecall/Sources/Views/DetailView.swift` |
| `TOCView` / `TOCParser` / `MarkdownSplitter` | Heading extraction + section splitting used by both macOS and iOS detail views | `ClaudeRecall/Sources/Views/TOCView.swift` |
| `FileFindBar` (macOS) | ⌘F find-bar UI with previous/next/escape behavior | `ClaudeRecall/Sources/Views/FileFindBar.swift` |
| `FindableReadableView` (macOS) | `NSTextView` wrapper showing the pre-rendered attributed document with NSRange-precise match highlighting | `ClaudeRecall/Sources/Views/FindableReadableView.swift` |
| `MarkdownEditorView` (macOS) | `NSTextView`-backed editor used in edit mode | `ClaudeRecall/Sources/Views/MarkdownEditorView.swift` |
| `UpdateBanner` (macOS) | Banner shown above the split view when `UpdateChecker.state == .available` | `ClaudeRecall/Sources/Views/UpdateBanner.swift` |
...
**Key Characteristics:**
- One `AppState` `@Observable` `@MainActor` instance owned by `ClaudeRecallApp` and injected via `.environment(appState)`; every view reads from it via `@Environment(AppState.self)`.
...
- Location: `ClaudeRecall/Sources/App/`
- Contains: `ClaudeRecallApp`, `AppDelegate`, `Notification.Name` extensions
- Depends on: `AppState`, `BookmarkStore`, `UpdateChecker`
- Used by: macOS / iOS runtime

**Models layer:**
- Purpose: State container + value types
- Location: `ClaudeRecall/Sources/Models/`
...
- Location: `ClaudeRecall/Sources/Views/`
...
- Location: `ClaudeRecall/Sources/Utilities/`
...
**Resources:**
- `ClaudeRecall/Sources/Resources/` — `Info.plist`, `PrivacyInfo.xcprivacy`, `Assets.xcassets`, and copies of the two entitlement files

## Data Flow

### Cold-start file open (Finder → Claude Recall, macOS)

1. macOS launches the bundle and invokes `AppDelegate.application(_:open:)` (`App/ClaudeRecallApp.swift` line 38).
2. If SwiftUI hasn't mounted yet (`viewReady == false`), the URL is appended to `AppDelegate.pendingFileURLs`.
3. `ClaudeRecallApp.body` builds the scene; `ContentView.onAppear` calls `AppDelegate.markViewReady()` (line 76).
...
### URL-scheme open (`clauderecall://open?...`)

1. macOS / iOS invokes the SwiftUI `.onOpenURL` handler (`App/ClaudeRecallApp.swift` line 62).
2. Non-file URLs are routed to `AppState.handleURL(_:)` (`Models/AppState.swift` line 265).
3. `handleURL` validates scheme = `clauderecall`, rejects path traversal (`..`) and non-absolute paths, standardizes the file URL, confirms `FileManager.fileExists`, then calls `openSingleFile`.
...
3. FSEvents fires on the `com.pelatari.claude-recall.filewatcher` dispatch queue; the C callback posts `.fileWatcherDidDetectChange` on the main queue.
...
**`ClaudeRecallApp` (`@main`):**
- Location: `ClaudeRecall/Sources/App/ClaudeRecallApp.swift` line 50
...
- Location: `ClaudeRecall/Sources/App/ClaudeRecallApp.swift` line 14
...
- **Threading:** All UI state mutation is `@MainActor` via `AppState`. Search runs on `DispatchQueue.global(qos: .userInitiated)` then bounces back to `.main`. File watching uses a dedicated `com.pelatari.claude-recall.filewatcher` dispatch queue and posts notifications on `.main`. Settings I/O is allowed off-main because `UserDefaults` and `NSUbiquitousKeyValueStore` are thread-safe.
...
- **No network in sandboxed builds:** `ClaudeRecall.entitlements` does not grant `com.apple.security.network.client`. The MAS binary cannot make outbound calls; `UpdateChecker.checkIfDue()` already short-circuits there, but any new network code must add the entitlement and update privacy disclosures.
...
**Why it's wrong:** The iOS target compiles the same `ClaudeRecall/Sources` tree and will fail at compile time.
...
**Why it's wrong:** The URL scheme is callable by any installed app via `open clauderecall://...`. Without the existing `..` and absolute-path checks, a malicious caller could try directory-traversal tricks.
...
**Strategy:** Best-effort, never crash, fall back to a sensible empty/default state. Claude Recall has no error reporting back-channel, so silent fallback is the deliberate policy for non-actionable failures.
